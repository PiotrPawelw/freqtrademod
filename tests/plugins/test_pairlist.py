...
# It has a mandatory parameter, and requires special handling, which
# happens in test_remotepairlist.
...
def test_remove_logs_for_pairs_already_in_blacklist(mocker, markets, static_pl_conf, caplog):
    ...
    for _ in range(3):
        ...
        # Ensure that the pair is removed from the white list, and properly
        # logged.
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    # Remock markets with shitcoinmarkets since get_patched_freqtradebot uses
    # the markets fixture
    mocker.patch.multiple(
        EXMS,
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    # Remove caching of ticker data to emulate changing volume by the time of
    # second call
    mocker.patch.multiple(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # AgeFilter and VolumePairList (require 2 days only, all should pass
        # age test)
        (
            [
                {"method": "VolumePairList", "number_assets": 5, "sort_key": "quoteVolume"},
        ...
        # AgeFilter and VolumePairList (require 10 days, all should fail age
        # test)
        (
            [
                {"method": "VolumePairList", "number_assets": 5, "sort_key": "quoteVolume"},
        ...
        # AgeFilter and VolumePairList (all pair listed > 2, all should fail
        # age test)
        (
            [
                {"method": "VolumePairList", "number_assets": 5, "sort_key": "quoteVolume"},
        ...
        # Hot is removed by precision_filter, Fuel by low_price_ratio, Ripple
        # by min_price.
        (
            [
        ...
        # Hot is removed by precision_filter, Fuel by low_price_ratio, Ethereum
        # by max_price.
        (
            [
                {"method": "VolumePairList", "number_assets": 6, "sort_key": "quoteVolume"},
        ...
        # HOT, XRP and FUEL whitelisted because they are below 1300
        # quoteVolume.
        (
            [
                {
        ...
        # HOT, XRP whitelisted because they are between 100 and 1300
        # quoteVolume.
        (
            [
                {
...
def test_VolumePairList_whitelist_gen(
    ...
    # Set whitelist_result to None if pairlist is invalid and should produce
    # exception
    if whitelist_result == "filter_at_the_beginning":
        ...
        # Verify length of pairlist matches (used for ShuffleFilter without
        # seed)
        ...
        for pairlist in pairlists:
            if (
                ...
                assert log_has_re(
                    r"^Removed .* from whitelist, because age .* is less than " r".* day.*",
                    caplog,
                )
            if (
                pairlist["method"] == "AgeFilter"
            ...
            if pairlist["method"] == "PriceFilter" and whitelist_result:
                assert (
                    ...
                    or log_has_re(
                        r"^Removed .* from whitelist, " r"because last price < .*%$",
                        caplog,
                    )
                    or log_has_re(
                        r"^Removed .* from whitelist, " r"because last price > .*%$",
                    ...
                    or log_has_re(
                        r"^Removed .* from whitelist, " r"because min value change of .*",
                        caplog,
                    )
                    or log_has_re(
...
def test_VolumePairList_range(
    ...
    # create candles for high volume with all candles high volume, but very
    # low price.
...
def test_PerformanceFilter_lookback(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    with time_machine.travel("2021-09-01 05:00:00 +00:00") as t:
        ...
        # Move to "outside" of lookback window, so original sorting is
        # restored.
...
@pytest.mark.parametrize(
    ...
    [
        (0.01, 0.99, 5),
        # Setting min rate_of_change to 5% removes all pairs from the
        # whitelist.
        (0.05, 0.0, 0),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Partial performance data missing and sorted between positive and
        # negative profit
        (
            [{"method": "StaticPairList"}, {"method": "PerformanceFilter"}],
            ["ETH/BTC", "TKN/BTC", "LTC/BTC"],
