...
def test_public_trades_mock_populate_dataframe_with_trades__check_orderflow(
    ...
    # Assert specific order flow values at the end of the DataFrame (excluding
    # last row)
    assert of_values[-1] == {
        "bid": 0.0,
        "ask": 1.0,
...
def test_public_trades_trades_mock_populate_dataframe_with_trades__check_trades(
    ...
    # Filter trades to those occurring after or at the same time as the first
    # DataFrame date
...
def test_public_trades_put_volume_profile_into_ohlcv_candles(public_trades_list_simple, candles):
    ...
    # Assert the delta value in the total-bid/delta response of the second
    # candle
    assert 0.14 == df.values.tolist()[1][2]

    # Alternative assertion using `.iat` accessor (assuming correct assignment
    # logic)
...
def test_public_trades_binned_big_sample_list(public_trades_list):
    ...
    # Assert the number of rows in the DataFrame (expected 23 for this bin
    # size)
    assert len(df) == 23
    ...
    # Assert the value in the last row of the DataFrame with the larger bin
    # size
