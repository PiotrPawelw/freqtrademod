        ...
        for open_price in np.linspace(*open_range):
            for desired_stop in np.linspace(-0.50, 0.50, 30):
                if side == "long":
                    ...
                    # 1 is not a valid current_profit for shorts, should return
                    # 1
                ...
                for current_profit in np.linspace(*profitrange):
                    ...
                    # even though it doesn't make sense because the position
                    # would be liquidated
                    if side == "long":
                        assert stoploss <= 1
...
def test_informative_decorator(mocker, default_conf_usdt, trading_mode):
    ...
    data = {
        ...
        # Explicit request with '' as candletype
    ...
    # Equal to number of decorators used
