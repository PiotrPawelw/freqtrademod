...
def test_adjust_stop_loss_short(fee):
    ...
    assert trade.initial_stop_loss_pct == -0.05
    assert trade.stop_loss_pct == -0.1
    # Liquidation price is lower than stoploss - so liquidation would trigger
    # first.
...
def test_Trade_object_idem():
    ...
    for item in trade:
        # Exclude private attributes and open_date (as it's not assigned a
        # default)
...
def test_recalc_trade_from_orders(fee):
    ...
    # Just to make sure full sell orders are ignored, let's calculate one more
    # time.
