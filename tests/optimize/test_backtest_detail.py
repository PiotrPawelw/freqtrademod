...
tc3 = BTContainer(
    data=[
        ...
        # enter trade 2 (signal on last candle)
        [4, 4975, 5000, 4950, 4962, 6172, 0, 0],
...
tc5 = BTContainer(
    data=[
        ...
        # enter trade (signal on last candle)
        [1, 5000, 5025, 4980, 4987, 6172, 0, 0],
...
# stop-loss: 10%, ROI: 10% (should not apply), -100% after 65 minutes
# (limits trade duration)
tc16 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
        [0, 5000, 5025, 4975, 4987, 6172, 1, 0],
...
# Uses open as sell-rate (special case) - since the roi-time is a multiple
# of the timeframe.
tc17 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
...
# stop-loss: 10%, ROI: 4%, stoploss adjusted candle 2, ROI adjusted in
# candle 3 (causing the sell)
tc24 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
        [0, 5000, 5050, 4950, 5000, 6172, 1, 0],
...
# Stoploss at 1%.
# Stoploss wins over Sell-signal (because sell-signal is acted on in the
# next candle)
tc25 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
        ...
        # Triggers stoploss + sellsignal
        [3, 5010, 5010, 4855, 5010, 6172, 0, 1],
...
tc26 = BTContainer(
    data=[
        ...
        # Triggers stoploss + sellsignal acted on
...
tc28 = BTContainer(
    data=[
        ...
        # enter trade (signal on last candle)
        [1, 5000, 5025, 4975, 4987, 6172, 0, 0, 0, 0],
        [2, 4987, 5012, 4986, 4986, 6172, 0, 0, 0, 0],
        [3, 5010, 5010, 4986, 5010, 6172, 0, 0, 0, 1],
        # Triggers stoploss + sellsignal acted on
        [4, 4990, 5010, 4855, 4995, 6172, 0, 0, 0, 0],
...
# Stoploss at 10% (irrelevant), ROI at 5% (will trigger) - Wins over
# Sell-signal
tc30 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
        ...
        # Triggers ROI, sell-signal acted on
        [4, 5010, 5251, 4855, 4995, 6172, 0, 0],
        [5, 4995, 4995, 4950, 4950, 6172, 0, 0],
    ],
    stop_loss=-0.10,
    roi={"0": 0.05},
...
tc33 = BTContainer(
    data=[
        ...
        [0, 5000, 5050, 4950, 5000, 6172, 1, 0],
        # enter trade (signal on last candle)
        [1, 5000, 5050, 5000, 5000, 6172, 0, 0],
...
tc34 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
        [0, 5000, 5050, 4950, 5000, 6172, 1, 0],
        # enter trade (signal on last candle) and stop
        [1, 5000, 5500, 4900, 4900, 6172, 0, 0],
...
# Test 39: Custom-entry-price below all candles should timeout - so no
# trade happens.
tc39 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
...
# Test 40: Custom-entry-price above all candles should have rate adjusted
# to "entry candle high"
tc40 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
        [0, 5000, 5050, 4950, 5000, 6172, 1, 0],
...
# Test 41: Custom-entry-price above all candles should have rate adjusted
# to "entry candle high"
tc41 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
...
# Test 51: Custom-entry-price below all candles - readjust order leaves
# order in place and timeout.
tc51 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
        [0, 5000, 5050, 4950, 5000, 6172, 1, 0],  # Enter long - place order
        # Order readjust - replace order
        [1, 5000, 5500, 4951, 5000, 6172, 0, 0],
        # Order readjust - maintain order
        [2, 4900, 5250, 4500, 5100, 6172, 0, 0],
...
# Test 53: Custom-entry-price short above all candles - readjust order -
# stoploss
tc53 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
        [0, 5000, 5050, 4950, 5000, 6172, 0, 0, 1, 0],
        # enter trade (signal on last candle)
        [1, 5000, 5200, 4951, 5000, 6172, 0, 0, 0, 0],
        [2, 4900, 5250, 4900, 5100, 6172, 0, 0, 0, 0],  # Order readjust
...
tc54 = BTContainer(
    data=[
        ...
        [1, 5000, 5000, 4951, 5000, 6172, 0, 0, 0, 0],
        # Enter short signal being ignored
        [2, 4910, 5150, 4910, 5100, 6172, 0, 0, 1, 0],
...
tc55 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
        [0, 5000, 5050, 4950, 5000, 6172, 0, 0, 1, 0],
        # Enter long signal being ignored
        [1, 5000, 5000, 4951, 5000, 6172, 1, 0, 0, 0],
...
tc56 = BTContainer(
    data=[
        ...
        [0, 5000, 5050, 4950, 5000, 6172, 1, 0, 0, 0],
        [1, 5000, 5000, 4951, 5000, 6172, 0, 0, 0, 0],
        # exit on stoploss - re-enter short
        [2, 4910, 5150, 4910, 5100, 6172, 0, 0, 1, 0],
...
# Causing the negative adjustment to cancel the unfilled order and exit
# partially
tc57 = BTContainer(
    data=[
        # D   O     H     L     C    V    EL XL ES Xs  BT
        ...
        # Enhance position, but won't fill
        [2, 4900, 5250, 4900, 5100, 6172, 0, 0, 0, 0],
        [3, 5100, 5100, 4650, 4750, 6172, 0, 0, 0, 0],
