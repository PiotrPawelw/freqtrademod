...
def test_data_to_dataframe_bt(default_conf, mocker, testdatadir) -> None:
    ...
    # Load strategy to compare the result between Backtesting function and
...
def test_backtest_one(default_conf, mocker, testdatadir) -> None:
    ...
    for _, t in results.iterrows():
        ...
        # check close trade rate aligns to close rate or is between high and
        # low
...
def test_backtest_one_detail_futures_funding_fees(
    ...
    # Additional counts will happen due each successful entry, which needs to
    # call this, too.
...
def test_backtest_pricecontours_protections(default_conf, fee, mocker, testdatadir) -> None:
    ...
    # results do not carry-over to the next run, which is not given by using
    # parametrize.
    patch_exchange(mocker)
