...
def test_throttle_sleep_time(mocker, default_conf, caplog) -> None:
    ...
    with time_machine.travel("2022-09-01 05:00:00 +00:00") as t:
        ...
        assert sleep_mock.call_count == 1
        # 300 (5m) - 60 (1m - see set time above) - 5 (duration of
        # throttled_func) = 235
        ...
        # Which should not get a throttle iteration to avoid late candle
        # fetching
        assert (
            worker._throttle(
        ...
        assert sleep_mock.call_count == 1
        # Time is slightly bigger than throttle secs due to the high timeframe
        # offset.
