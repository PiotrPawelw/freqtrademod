...
# pragma pylint: disable=protected-access, too-many-lines, invalid-name,
# too-many-arguments
...
def test_update_trade_state(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    caplog.clear()
    # Add datetime explicitly since sqlalchemy defaults apply only once
    # written to database
...
def test_update_trade_state_sell(
    ...
    # Wallet needs to be updated after closing a limit-sell order to re-enable
    # buying
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # different fee currency on both trades, fee is average of both trade's
        # fee
...
def test_apply_fee_conditional_multibuy(
    ...
    trade.orders.append(order)
    # Add additional order - this should NOT eat into dust unless the wallet
    # was bigger already.
    order1 = Order(
        ft_order_side="buy",
        order_id="100",
    ...
    walletmock.reset_mock()
    # The new trade amount will be 2x amount - fee / wallet will have to be
    # adapted to this.
...
def test_check_depth_of_market(default_conf_usdt, mocker, order_book_l2) -> None:
    ...
    default_conf_usdt["exchange"]["name"] = "binance"
    default_conf_usdt["entry_pricing"]["check_depth_of_market"]["enabled"] = True
    # delta is 100 which is impossible to reach. hence function will return
    # false
...
def test_startup_update_open_orders(mocker, default_conf_usdt, fee, caplog, is_short):
    ...
    # Orders which are no longer found after X days should be assumed as
    # canceled.
    freqtrade.startup_update_open_orders()
...
def test_handle_onexchange_order(mocker, default_conf_usdt, limit_order, is_short, caplog):
    ...
    # Update trade state is called twice, once for the known and once for the
    # unknown order.
    assert mock_uts.call_count == 2
    assert mock_fo.call_count == 1
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # tuple 2 - amount, open_rate, stake_amount, cumulative_profit,
        # realized_profit, rel_profit
        (
