...
class RPC:
    ...
    def _rpc_trade_statistics(
        ...
        # Doing the sum is not right - overall profit needs to be based on
        # initial capital
    ...
    def __balance_get_est_stake(
        ...
        if coin == self._freqtrade.exchange.get_proxy_coin():
            ...
            if is_futures:
                # in Futures, "total" includes the locked stake, and therefore
                # all positions
    ...
    def _convert_dataframe_to_dict(
        ...
        if has_content:
            ...
            # Move signal close to separate column when signal for easy
            # plotting
    ...
    def _rpc_plot_config_with_strategy(config: Config) -> dict[str, Any]:
        ...
        strategy = StrategyResolver.load_strategy(config)
        # Manually load hyperparameters, as we don't call the bot-start
        # callback.
