...
class Exchange:
    # Parameters to add directly to buy/sell calls (like agreeing to trading
    # agreement)
    _params: dict = {}
    ...
    _ft_has_default: FtHas = {
        ...
        # Used for stoploss_on_exchange response parsing
        "stop_price_prop": "stopLossPrice",
        "stoploss_order_types": {},
        "order_time_in_force": ["GTC"],
        "ohlcv_params": {},
        "ohlcv_candle_limit": 500,
        # Some exchanges (Kraken) don't provide history via ohlcv
        "ohlcv_has_history": True,
        "ohlcv_partial_candle": True,
        "ohlcv_require_since": False,
        # Check https://github.com/ccxt/ccxt/issues/10767 for removal of
        # ohlcv_volume_currency
        ...
        # Expected to be in the format {"fetchOHLCV": True} or {"fetchOHLCV":
        # False}
    ...
    def __init__(
        ...
        *,
        ...
        # Shouldn't be too high either, as it'll freeze UI updates in case of
        # open orders.
    ...
    def _init_ccxt(
        ...
        if self._ccxt_params:
            # Inject static options after the above output to not confuse
            # users.
    ...
    def reload_markets(self, force: bool = False, *, load_leverage_tiers: bool = True) -> None:
        ...
        try:
            ...
            # Assign options array, as it contains some temporary information
            # from the exchange.
    ...
    def create_dry_run_order(
        ...
        # Copy order and close it - so the returned order is open unless it's a
        # market order
        return dry_order
    ...
    def get_dry_market_fill_price(
        ...
        if self.exchange_has("fetchL2OrderBook"):
            ...
            for book_entry in orderbook[ob_type]:
                ...
                # If remaining_amount wasn't consumed completely (break was not
                # called)
                filled_value += remaining_amount * book_entry_price
    ...
    def _dry_is_price_crossed(
        ...
        try:
            ...
            # Ignore empty orderbooks when filling - can be filled with the
            # next iteration.
            pass
        return False
    ...
    def create_order(
        ...
        *,
        ...
        try:
            # Set the precision for amount and price(rate) as accepted by the
            # exchange
    ...
    def _get_stop_limit_rate(self, stop_price: float, order_types: dict, side: str) -> float:
        # Limit price threshold: As limit price should always be below
        # stop-price
    ...
    def _get_stop_params(self, side: BuySell, ordertype: str, stop_price: float) -> dict:
        params = self._params.copy()
        # Verify if stopPrice works for your exchange, else configure
        # stop_price_param
    ...
    async def _async_get_historic_ohlcv(
        ...
        data: list = []
        # Chunk requests into batches of 100 to avoid overwhelming ccxt
        # Throttling
        ...
        if cache and (pair, timeframe, candle_type) in self._klines:
            ...
            if self._exchange_ws:
                ...
                if (
                    ...
                    # Also, we check if the last refresh time is no more than
                    # half the candle ago.
                ...
                if is_in_cache:
                    ...
                    if not all_stored_ticks_df.empty:
                        if (
                            ...
                            # only use cached if it's closer than
                            # first_candle_ms
                            since_ms = (
                                last_cached_ms
            ...
            try:
                ...
                if t:
                    ...
                    if from_id == from_id_next or t[-1][0] > until:
                        ...
                        # Reached the end of the defined-download period - add
                        # last trade as well.
                        if has_overlap:
                            trades.extend(t[-1:])
                        break
    ...
    def _fetch_and_calculate_funding_fees(
        ...
        try:
            # we can't assume we always get histories - for example during
            # exchange downtimes
    ...
    def calculate_funding_fees(
        ...
        if isnan(fees):
            fees = 0.0
        # Negate fees for longs as funding_fees expects it this way based on
        # live endpoints.
    ...
    def get_maintenance_ratio_and_amt(
        ...
        if (
            ...
            # describes the min amt for a tier, and the lowest tier will always
            # go down to 0
        else:
