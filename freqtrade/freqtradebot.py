...
class FreqtradeBot(LoggingMixin):
    ...
    def __init__(self, config: Config) -> None:
        ...
        # Remove credentials from original exchange config to avoid accidental
        # credential exposure
        remove_exchange_credentials(config["exchange"], True)

        self.strategy: IStrategy = StrategyResolver.load_strategy(self.config)

        # Check config consistency here since strategies can set certain
        # options
        ...
        if self.trading_mode == TradingMode.FUTURES:
            ...
            # However, this reduces the precision - and might therefore lead to
            # problems.
        ...
        # Initialize protections AFTER bot start - otherwise parameters are not
        # loaded.
    ...
    def process(self) -> None:
        ...
        # Check whether markets have to be reloaded and reload them when it's
        # needed
        ...
        with self._exit_lock:
            # Check for exchange cancellations, timeouts and user requested
            # replace
            self.manage_open_orders()

        # Protect from collisions with force_exit.
        # Without this, freqtrade may try to recreate stoploss_on_exchange orders
        # while exiting is in process, since telegram messages arrive in an
        # different thread.
        with self._exit_lock:
        ...
        # Check if we need to adjust our current positions before attempting to
        # enter new trades.
    ...
    def _refresh_active_whitelist(self, trades: list[Trade] | None = None) -> list[str]:
        ...
        if trades:
            ...
            # It ensures that candle (OHLCV) data are downloaded for open
            # trades as well
    ...
    def startup_update_open_orders(self):
        ...
        if self.config["dry_run"] or self.config["exchange"].get("skip_open_order_update", False):
            # Updating open orders in dry-run does not make sense and will
            # fail.
            return
        ...
        for order in orders:
            try:
                ...
                if not order.trade:
                    ...
                    # This can only incur on sqlite, which doesn't enforce
                    # foreign constraints.
                    logger.warning(
    ...
    def create_trade(self, pair: str) -> bool:
        ...
        # but it is still used here to prevent opening too many trades within
        # one iteration
    ...
    def execute_entry(
        ...
        *,
        ...
        if trade is None:
            ...
            # This is additional entry, we reset fee_open_currency so timeout
            # checking can work
            trade.is_open = True
    ...
    def handle_stoploss_on_exchange(self, trade: Trade) -> bool:
        ...
        if not trade.has_open_position or not trade.is_open:
            # The trade can be closed already (sell-order fill confirmation
            # came in this iteration)
            return False

        # If enter order is fulfilled but there is no stoploss, we add a
        ...
        if len(stoploss_orders) == 0:
            ...
            if self.create_stoploss_order(trade=trade, stop_price=stop_price):
                ...
                # in which case the trade will be closed - which we must check
                # below.
                return False
    ...
    def manage_trade_stoploss_orders(self, trade: Trade, stoploss_orders: list[CcxtOrder]):
        ...
        if len(active_sl_orders) > 0:
            ...
            # Triggered Orders are now real orders - so don't replace stoploss
            # anymore
            if (
                trade.is_open
    ...
    def replace_order_failed(self, trade: Trade, msg: str) -> None:
        ...
        if trade.nr_of_successful_entries == 0:
            # this is the first entry and we didn't get filled yet, delete
            # trade
    ...
    def handle_cancel_enter(
        ...
        if order["status"] not in constants.NON_OPEN_EXCHANGE_STATES:
            ...
            # if replacing, retry fetching the order 3 times if the status is
            # not what we need
            if replacing:
                retry_count = 0
                while (
        ...
        if isclose(filled_amount, 0.0, abs_tol=constants.MATH_CLOSE_PREC):
            ...
            # if trade is not partially completed and it's the only order, just
            # delete the trade
    ...
    def _safe_exit_amount(self, trade: Trade, pair: str, amount: float) -> float:
        ...
        self.wallets.update()
        if self.trading_mode == TradingMode.FUTURES:
            # A safe exit amount isn't needed for futures, you can just
    ...
    def _notify_exit(
        ...
        # second condition is for mypy only; order will always be passed during
        # sub trade
    ...
    def apply_fee_conditional(
        ...
        if trade.nr_of_successful_entries >= 1 and order_obj.ft_order_side == trade.entry_side:
            # In case of re-entry's, trade.amount doesn't contain the amount of
            # the last entry.
            amount_ = trade.amount + amount
    ...
    def get_real_amount(self, trade: Trade, order: CcxtOrder, order_obj: Order) -> float | None:
        ...
        if self.exchange.order_has_fee(order):
            ...
            if fee_rate is None or fee_rate < 0.02:
                ...
                # due to multiple trades
                # (https://github.com/ccxt/ccxt/issues/8025)
