...
class Order(ModelBase):
    ...
    def close_bt_order(self, close_date: datetime, trade: "LocalTrade"):
        ...
        # Assumes backtesting will use date_last_filled_utc to calculate future
        # funding fees.
...
class LocalTrade:
    ...
    def update_trade(self, order: Order, recalculating: bool = False) -> None:
        ...
        if order.ft_order_side != self.entry_side:
            ...
            if isclose(order.safe_amount_after_fee, amount_tr, abs_tol=MATH_CLOSE_PREC) or (
                ...
                # When recalculating a trade, only coming out to 0 can force a
                # close
    ...
    def update_fee(
        ...
        if self.entry_side == side and self.fee_open_currency is None:
            ...
            if fee_rate is not None:
                ...
                # Assume close-fee will fall into the same fee category and
    ...
    def recalc_trade_from_orders(self, *, is_closing: bool = False):
        ...
        # Total stake after all buy orders (does not subtract!)
        total_stake = 0.0
        ...
        if current_amount_tr > 0.0:
            # Trade is still open
            # Leverage not updated, as we don't allow changing leverage through
            # DCA at the moment.
    ...
    def remove_bt_trade(trade):
        ...
        # It might need to be removed so the trade "counts" as open for this
        # candle.
        LocalTrade.bt_open_open_trade_count_candle -= 1
