...
class Backtesting:
    ...
    def __init__(self, config: Config, exchange: Exchange | None = None) -> None:
        ...
        # Add maximum startup candle count to configuration for informative
        # pairs support
        ...
        # strategies which define "can_short=True" will fail to load in Spot
        # mode.
    ...
    def load_bt_data_detail(self) -> None:
        ...
        if self.trading_mode == TradingMode.FUTURES:
            ...
            # For simplicity, assign to CandleType.Mark (might contain index
            # candles!)
    ...
    def _get_close_rate_for_stoploss(
        ...
        if exit_.exit_type == ExitType.TRAILING_STOP_LOSS and trade_dur == 0:
            if (
                ...
                # Worst case: price reaches stop_positive_offset and dives
                # down.
    ...
    def _get_close_rate_for_roi(
        ...
        if roi is not None and roi_entry is not None:
            ...
            if trade_dur == 0 and (
                ...
                # If open_rate is < open, only allow exits below the close on
                # red candles.
                raise ValueError("Opening candle ROI on red candles.")

            # Use the maximum between close_rate and low as we
            # cannot exit outside of a candle.
            # Applies when a new ROI setting comes in place and the whole
            # candle is above that.
    ...
    def _get_adjust_trade_entry_for_candle(
        ...
        if stake_amount is not None and stake_amount < 0.0:
            ...
            if pos_trade is not None:
                ...
                # If the order was filled and for the full trade amount, we
                # need to close the trade.
    ...
    def _get_exit_for_signal(
        ...
        if exit_.exit_flag:
            ...
            try:
                ...
                return None
            # call the custom exit price,with default value as previous
            # close_rate
            ...
            if exit_.exit_type in (
                ...
                if order_type == "limit":
                    ...
                    # freqtrade does not support this in live, and the order
    ...
    def _enter_trade(
        ...
        # let's call the custom entry price, using the open price as default
        # price
    ...
    def check_order_replace(
        ...
        if order.side == trade.entry_side and current_time > order.order_date_utc:
            ...
            if requested_rate == order.ft_price:
                # assumption: there can't be multiple open entry orders at any
                # given time
                return False
            else:
                del trade.orders[trade.orders.index(order)]
            ...
            if requested_rate:
                ...
                # Delete trade if no successful entries happened (if placing
                # the new order failed)
    ...
    def validate_row(
        ...
        # Waits until the time-counter reaches the start of the data for this
        # pair.
    ...
    def backtest_loop(
        ...
        if not self._can_short or trade_dir is None:
            # No need to reverse position if shorting is disabled or there's no
            # new signal
            ...
            for _ in (0, 1):
                ...
                if not a or a == trade_dir:
                    # the trade didn't close or position change is in the same
                    # direction
                    break
    ...
    def time_pair_generator(
        ...
        for current_time in self.time_generator(start_date, end_date):
            ...
            # when timeframe-detail is used and trades close within the opening
            # candle.
        ...
        for current_time, pair in self.time_pair_generator(
            ...
            if (
                ...
                if detail_data is None or len(detail_data) == 0:
                    # Fall back to "regular" data if no detail data was found
                    # for this candle
                    self.dataprovider._set_dataframe_max_date(current_time)
                ...
                for det_row in detail_data[HEADERS].values.tolist():
                    ...
                    if pair_has_open_trades and not len(LocalTrade.bt_trades_open_pp[pair]) > 0:
                        # Auto-lock pair for the rest of the candle if the
    ...
    def backtest_one_strategy(
        ...
        # Trim startup period from analyzed dataframe
        # This only used to determine if trimming would result in an empty
        # dataframe
        ...
        # Backtesting will re-trim the dataframes after entry/exit signal
        # generation.
    ...
    def load_prior_backtest(self):
        ...
        # This can be circumvented in certain instances in combination with
    ...
    def start(self) -> None:
        ...
        for strat in self.strategylist:
            if self.results and strat.get_strategy_name() in self.results["strategy"]:
                # When previous result hash matches - reuse that result and
                # skip backtesting.
        ...
        # Results may be mixed up now. Sort them so they follow --strategy-list
        # order.
