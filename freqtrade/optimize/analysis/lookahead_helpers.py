...
class LookaheadAnalysisSubFunctions:
    ...
    def export_to_csv(config: dict[str, Any], lookahead_analysis: list[LookaheadAnalysis]):
        ...
        if Path(config["lookahead_analysis_exportfilename"]).exists():
            ...
            # Create a new empty DataFrame with the desired column names and
            # set the index
            csv_df = pd.DataFrame(
                columns=[
                    "filename",
    ...
    def calculate_config_overrides(config: Config):
        if config.get("enable_protections", False):
            # if protections are used globally, they can produce false
            # positives.
    ...
    def start(config: Config):
        ...
        # check if strategies can be properly loaded, only check them if they
        # can be.
