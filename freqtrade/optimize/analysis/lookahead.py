...
class LookaheadAnalysis(BaseAnalysis):
    ...
    # analyzes two data frames with processed indicators and shows differences
    # between them.
        ...
        if full_df_cut.shape[0] != 0 and cut_df_cut.shape[0] != 0:
            ...
            if compare_df.shape[0] > 0:
                for col_name, values in compare_df.items():
                    ...
                    # compare_df now comprises tuples with [1] having either
                    # 'self' or 'other'
    ...
    def fill_entry_and_exit_varHolders(self, result_row):
        ...
        # to_dt needs +1 candle since it will always exit/force-exit trades on
        # the last candle
    ...
    # now we analyze a full trade of full_varholder and look for analyze its
    # bias
    def analyze_row(self, idx: int, result_row):
        # if force-sold, ignore this signal since here it will unconditionally
        # exit.
